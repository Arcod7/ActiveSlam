...
_ODOM_BUFFER = 64   # keep last 64 odometry stamps (~0.64 s at 100 Hz)
...
class DepthFix(Node):
    def __init__(self):
        ...
        # Ring buffer of builtin_interfaces.msg.Time stamps from odometry
        ...
        self._pub = self.create_publisher(Image, '/sensor_msgs/image_depth_fixed', 10)
    ...
    def _depth_cb(self, msg: Image):
        ...
        # --- 2. Replace timestamp with nearest past odometry stamp ---
        best = self._best_odom_stamp(msg.header.stamp)
        if best is not None:
            fixed.header.stamp = best

        self._pub.publish(fixed)
