...
- **explains each option** in a description pane as you move through them, with
  the values it can take pinned to the bottom of the right-hand panel, under a
  rule (bounds and arrow-key step, for the numeric ones). `Space` cycles a
  value forward, the same as Right.
...
- **hands you its own command line.** The full single-shot
  `ros2 launch bringup demo.launch.py …` equivalent is far too long for one
  row, so the second row carries a `copy launch command` button instead —
  click it, or press `C`, and the command goes to the clipboard (`wl-copy`,
  `xclip` or `xsel`, falling back to the terminal's own OSC 52 handler, which
  works over SSH). Only options differing from their defaults are listed.
...
- **drives the vehicle straight from the option screen** — the drive keys are
  ...
  strafe, `A/D` yaw, `Z/X` up/down, `F` stop (QWERTY; an AZERTY option
...
- **swims to a target point** — the *Mode* option's third value, `goto`. The
  ...
  too: when the uncertainty ratio `U_r = D(Σ)/D(Σ_allow)` crosses the trigger,
  `revisit_planner` takes the goal topic and drives a loop-closing detour, and
  the launcher resends the target point as soon as the detour ends — the run
  reaches the point *and* keeps its pose uncertainty bounded. The footer marks
  the point as on hold while that happens, and the trigger/resume ratios sit
  under the live `d-opt`, `U_r` and `ANEES` readings in the right-hand panel.
